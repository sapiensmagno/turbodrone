...
def telemetry_to_flat_sample(t: Any) -> Dict[str, Any]:
    ...
    return {
        ...
        "flow_omega_x_rad": None if flow is None else float(getattr(flow, "omega_x_rad", 0.0)),
        "flow_omega_y_rad": None if flow is None else float(getattr(flow, "omega_y_rad", 0.0)),
        "flow_omega_z_rad": None if flow is None else float(getattr(flow, "omega_z_rad", 0.0)),
